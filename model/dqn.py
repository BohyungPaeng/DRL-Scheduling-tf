...
class BaseNetwork(tf.Module):
    def __init__(self, sess, input_dim, action_dim, update_option, name, optimizer, tau,
                 initializer=VarianceScaling(mode="fan_in")):
        """
        ...
        # build network
        self.build(name)
        # self.network_param = [v for v in tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.MODEL_VARIABLES) if
        #                       name in v.name and
        #                       "target" not in v.name]
        self.network_param = [v for v in tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.GLOBAL_VARIABLES) if
                              name in v.name and
                              "target" not in v.name]

        # build target
        self.build_target("target_%s" % name)
        self.target_param = [v for v in tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.GLOBAL_VARIABLES) if
                             name in v.name and
                             "target" in v.name]
        self.gradients = None
    ...
    def create_update_op(self):
        import time
        st = time.time()
        print('Create update op:', st)

        print(len(self.target_param), len(self.network_param))
        if self.update_option == "soft_update":
            update_op = [tf.compat.v1.assign(target_param, (1 - self.tau) * target_param + self.tau * network_param)
                         for target_param, network_param in zip(self.target_param, self.network_param)]
        else:
            update_op = [tf.compat.v1.assign(target_param, network_param)
                         for target_param, network_param in zip(self.target_param, self.network_param)]
    ...
    def init_summary(self, weight_hist, summary_dir, name):
        ...
        if summary_dir:
            ...
            self.summary_writer = tf.compat.v1.summary.FileWriter(summary_dir + '/scalar')
            if self.is_train and self.hist_freq: self.summary_writer_hist = tf.compat.v1.summary.FileWriter(
                summary_dir + '/hist')
            ...
            for var in var_list:
                ...
                if 'weights' in var.op.name or 'kernel' in var.op.name or 'bias' in var.op.name:
                    ...
                    with tf.compat.v1.variable_scope(name, reuse=True):
                        weight = tf.compat.v1.get_variable(weight_name)
                    summary_list.append(tf.compat.v1.summary.histogram(var.op.name, weight))
        summary_list.extend([
            # tf.summary.histogram('fc1_weight ' % name , (weights)),
            # tf.summary.histogram('fc1_bias' , b),
            tf.compat.v1.summary.scalar('logits/max_predicted_q_value', tf.reduce_max(self.predicted_Q)),
            # tf.summary.scalar('loss/batch_loss_%s' % name, self.loss),
            tf.compat.v1.summary.scalar('loss/batch_loss', self.loss),
        ])
        if self.is_duel:
            summary_list.extend([
                tf.compat.v1.summary.scalar('logits/value_estimated', tf.reduce_max(self.value[0])),
                tf.compat.v1.summary.scalar('logits/max_predicted_a_value', tf.reduce_max(self.adv[0]))
            ])

        print('Setting {} length of default summary'.format(len(summary_list)))
        self.summary = tf.compat.v1.summary.merge(summary_list)


class PDQN(BaseNetwork):
    def __init__(self, sess, input_dim, action_dim, auxin_dim, tau, optimizer, name, global_step,
                 is_duel=False, is_train=True, h_dims=[64, 32], summary_dir=None, weight_hist=False):
        """
        ...
        super(PDQN, self).__init__(sess, input_dim, action_dim, update_option="soft_update",
                                   name=self.net_name, optimizer=optimizer, tau=tau)

        self.update_target = self.create_update_op()

        self.is_training = tf.compat.v1.placeholder(dtype=tf.bool, name="bn_is_train")
        ...
        self.target_Q = tf.compat.v1.placeholder(tf.float32, shape=[None, 1], name="target_q")

        self.action = tf.compat.v1.placeholder(tf.float32, shape=[None, self.action_dim], name="selected_action")
        self.predicted_Q = tf.reduce_sum(tf.multiply(self.action, self.net), axis=1, keepdims=True)  # batch_size
    ...
    def base_encoder_cells(self, x, name='', reuse=False):
        if len(self.input_dim) == 2:
            ...
            for i in range(self.input_dim[0]):
                ...
                net_temp = self._base_encoder_cells(_input, name=name, reuse=tf.compat.v1.AUTO_REUSE)
                if net is None:
                    net = net_temp
                else:
                    net = tf.concat([net, net_temp], 1)
        else:
            net = self._base_encoder_cells(x, name=name, reuse=reuse)
        return net

    def _base_encoder_cells(self, x, name='', reuse=False):
        with tf.compat.v1.variable_scope(name, reuse=reuse):
            net = tf.identity(x)
            for l in range(len(self.h_dims)):
                h_dim = self.h_dims[l]
                net = tf.compat.v1.layers.dense(net, h_dim, activation=tf.nn.relu,
    ...
    def value_layer(self, net, name='', reuse=False):
        with tf.compat.v1.variable_scope(name, reuse=reuse):
            ...
            net = tf.compat.v1.layers.dense(net, activation=None, units=self.action_dim,
                                            kernel_initializer=tf.compat.v1.random_uniform_initializer(-3e-3, 3e-3),
                                            bias_initializer=tf.compat.v1.constant_initializer(.0, dtype=tf.float32),
                                            name="q", reuse=reuse)
        return net
    ...
    def build(self, name):
        self.x = tf.compat.v1.placeholder(dtype=tf.float32, shape=[None, *self.input_dim], name="%s_input" % name)
        net = self.base_encoder_cells(self.x, name)
        if self.auxin_dim > 0:
            self.auxin = tf.compat.v1.placeholder(dtype=tf.float32, shape=[None, self.auxin_dim], name="%s_auxin" % name)
        ...
        if self.is_duel:
            ...
            self.net = self.value + (self.adv - tf.reduce_mean(self.adv, axis=[1, ], keepdims=True))

        else:
            self.net = self.value_layer(net, name=name)

    def build_target(self, name):
        self.target_x = tf.compat.v1.placeholder(dtype=tf.float32, shape=[None, *self.input_dim],
                                    name="%s_input" % name)
        net = self.base_encoder_cells(self.target_x, name)
        if self.auxin_dim > 0:
            self.target_auxin = tf.compat.v1.placeholder(dtype=tf.float32,
                                            shape=[None, self.auxin_dim], name="%s_auxin" % name)
        ...
        if self.is_duel:
            ...
            self.target_net = value + (adv - tf.reduce_mean(adv, axis=[1, ], keepdims=True))
    ...
    def critic_predict(self, state, auxin, feasibility=None):
        ...
        return self.sess.run(self.net, feed_dict=feed_dict)
    ...
    def _loss(self):
        ...
        delta = 0.5
        loss = tf.compat.v1.where(tf.abs(diff) < delta, 0.5 * tf.square(diff),
                                  delta * tf.abs(diff) - 0.5 * (delta ** 2))
        # For Prioritized Replay buffer(Importance Sampling)
        self.weight = tf.compat.v1.placeholder(tf.float32, shape=[None, 1], name="weight_is")
    ...
    def update_critic(self):
        """
            instead of creating update_op tensor architecture(1.x),
            eager executing target param update
        """
